...
@section('content')
    ...
    <div class="page-header-container">
        <div class="page-title-group">
            ...
            <p>Rincian data pribadi, informasi akademik, dan profil siswa</p>
        </div>
    </div>
    ...
    <div class="detail-card">
        <div class="profile-header">
            @php
                $words = explode(' ', trim($siswa->nama_siswa));
        ...
        <div class="info-grid">
            <div class="info-item">
                <div class="label">NIS (Nomor Induk Siswa)</div>
                <div class="value">{{ $siswa->nis ?? '-' }}</div>
            </div>

            <div class="info-item">
                <div class="label">NISN</div>
                <div class="value" style="font-family:monospace; color:#2563eb;">{{ $siswa->nisn }}</div>
            </div>

            <div class="info-item">
                <div class="label">Nama Lengkap</div>
                <div class="value">{{ $siswa->nama_siswa }}</div>
            </div>
            ...
            <div class="info-item">
                ...
                <div class="value" style="color:#3b5490;">{{ $siswa->kelas->nama_kelas ?? '-' }}</div>
            </div>

            <div class="info-item">
                <div class="label">Tempat, Tanggal Lahir</div>
                <div class="value">
                    @if($siswa->kota_lahir || $siswa->tanggal_lahir)
                        {{ $siswa->kota_lahir ?? '' }}{{ $siswa->kota_lahir && $siswa->tanggal_lahir ? ', ' : '' }}{{ $siswa->tanggal_lahir ? \Carbon\Carbon::parse($siswa->tanggal_lahir)->format('d F Y') : '' }}
                    @else
                        -
                    @endif
                </div>
            ...
            <div class="info-item" style="grid-column: 1 / -1;">
                ...
                <div class="value">{{ $siswa->alamat_lengkap ?? '-' }}</div>
            </div>
        </div>

        <div class="action-row">
            <a href="{{ route('siswa.index') }}" class="btn-act btn-act-back">
                <i class="fa-solid fa-arrow-left"></i> Kembali ke Daftar Siswa
            </a>
            <a href="{{ route('siswa.edit', $siswa->id_siswa) }}" class="btn-act btn-act-edit">
                <i class="fa-solid fa-pen-to-square"></i> Edit Data Siswa
            </a>
            <form action="{{ route('siswa.destroy', $siswa->id_siswa) }}" method="POST" style="display:inline-block;">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn-act btn-act-delete" onclick="return confirm('Apakah Anda yakin ingin menghapus data siswa {{ addslashes($siswa->nama_siswa) }}?')">
                    <i class="fa-solid fa-trash-can"></i> Hapus Siswa
                </button>
            </form>
        </div>
    </div>
