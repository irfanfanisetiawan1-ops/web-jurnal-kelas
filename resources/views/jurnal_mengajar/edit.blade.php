...
<head>
    ...
    <title>Edit Jurnal Mengajar — Jurnal Kelas</title>
    ...
    <style>
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-submit:hover {
            ...
            box-shadow: 0 8px 20px -2px rgba(217, 119, 6, 0.45);
        }
...
<div class="container">
    ...
    <div class="card">
        <div class="card-header">
            <div class="card-icon">
                <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
            </div>
            <div class="card-title">
                <h1>Edit Jurnal Mengajar</h1>
                <p>Perbarui informasi aktivitas mengajar & data presensi siswa</p>
            </div>
        </div>

        <form action="{{ route('jurnal-mengajar.update', $jurnal->id_jurnal) }}" method="POST" id="editForm">
            @csrf
            @method('PUT')
            ...
            <div class="form-grid">
                ...
                <div class="form-group full-width">
                    ...
                    <select name="id_jadwal" id="id_jadwal" class="@error('id_jadwal') input-error @enderror" required onchange="loadSiswaByJadwal(this.value)">
                        <option value="" disabled>-- Pilih Jadwal Pelajaran --</option>
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="tanggal" id="tanggal" value="{{ old('tanggal', $jurnal->tanggal) }}" class="@error('tanggal') input-error @enderror" required>
                    @error('tanggal')
                        <div class="error-message">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="status_kehadiran_guru" id="status_kehadiran_guru" class="@error('status_kehadiran_guru') input-error @enderror" required>
                        @foreach($statusOptions as $st)
                            <option value="{{ $st }}" {{ old('status_kehadiran_guru', $jurnal->status_kehadiran_guru) == $st ? 'selected' : '' }}>{{ $st }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group full-width">
                    ...
                    <textarea name="materi" id="materi" placeholder="Tuliskan pokok pembahasan / materi yang diajarkan..." class="@error('materi') input-error @enderror" required>{{ old('materi', $jurnal->materi) }}</textarea>
                    @error('materi')
                        <div class="error-message">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group full-width">
                    ...
                    <textarea name="catatan" id="catatan" placeholder="Catatan kondisi kelas, keaktifan siswa, atau kejadian penting..." class="@error('catatan') input-error @enderror">{{ old('catatan', $jurnal->catatan) }}</textarea>
                    @error('catatan')
                        <div class="error-message">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="section-divider">
                ...
                <p>Tandai siswa yang Sakit, Izin, atau Alpa</p>
            </div>
            ...
            <div id="siswaPresensiContainer">
                @if(count($siswas) > 0)
                    <div style="margin-bottom:8px; display:flex; justify-content:space-between; align-items:center;">
                        <span style="font-size:13px; font-weight:800; color:#3730a3; background:#e0e7ff; padding:3px 10px; border-radius:8px;">
                            Kelas: {{ $jurnal->jadwal->kelas->nama_kelas ?? '-' }} ({{ count($siswas) }} Siswa)
                        </span>
                        ...
                        <table class="absence-table">
                            ...
                            <tbody>
                                @foreach($siswas as $idx => $s)
                                    @php
                                        $currentStatus = $existingKetidakhadiran[$s->id_siswa] ?? 'Hadir';
                                    @endphp
                                    <tr>
                                        <td style="font-weight:700; color:#94a3b8;">{{ $idx + 1 }}</td>
                                        <td style="font-weight:600; color:#64748b;">{{ $s->nisn }}</td>
                                        <td style="font-weight:700; color:#0f172a;">{{ $s->nama_siswa }}</td>
                                        <td style="text-align: right;">
                                            <div class="status-radio-group" style="justify-content: flex-end;">
                                                <label class="status-radio-label">
                                                    <input type="radio" name="siswa_status_{{ $s->id_siswa }}" value="Hadir" {{ $currentStatus == 'Hadir' ? 'checked' : '' }} onchange="toggleKetidakhadiranInput({{ $idx }}, {{ $s->id_siswa }}, 'Hadir')">
                                                    <span style="color:#15803d;">Hadir</span>
                                                </label>
                                                <label class="status-radio-label">
                                                    <input type="radio" name="siswa_status_{{ $s->id_siswa }}" value="Sakit" {{ $currentStatus == 'Sakit' ? 'checked' : '' }} onchange="toggleKetidakhadiranInput({{ $idx }}, {{ $s->id_siswa }}, 'Sakit')">
                                                    <span style="color:#1d4ed8;">Sakit</span>
                                                </label>
                                                <label class="status-radio-label">
                                                    <input type="radio" name="siswa_status_{{ $s->id_siswa }}" value="Izin" {{ $currentStatus == 'Izin' ? 'checked' : '' }} onchange="toggleKetidakhadiranInput({{ $idx }}, {{ $s->id_siswa }}, 'Izin')">
                                                    <span style="color:#d97706;">Izin</span>
                                                </label>
                                                <label class="status-radio-label">
                                                    <input type="radio" name="siswa_status_{{ $s->id_siswa }}" value="Alpa" {{ $currentStatus == 'Alpa' ? 'checked' : '' }} onchange="toggleKetidakhadiranInput({{ $idx }}, {{ $s->id_siswa }}, 'Alpa')">
                                                    <span style="color:#e11d48;">Alpa</span>
                                                </label>
                                            </div>
                                            <div id="input_wrap_{{ $s->id_siswa }}">
                                                @if($currentStatus != 'Hadir')
                                                    <input type="hidden" name="ketidakhadiran[{{ $s->id_siswa }}][id_siswa]" value="{{ $s->id_siswa }}">
                                                    <input type="hidden" name="ketidakhadiran[{{ $s->id_siswa }}][keterangan]" value="{{ $currentStatus }}">
                                                @endif
                                            </div>
                                        </td>
                                    </tr>
                ...
                @else
                    <div style="background: #f8fafc; border: 1.5px dashed #cbd5e1; border-radius: 14px; padding: 24px; text-align: center; color: #64748b;">
                        <p style="font-size: 13.5px; font-weight: 600;">Silakan pilih Jadwal Pelajaran terlebih dahulu.</p>
                    </div>
                @endif
            </div>
        ...
        if (form) {
            form.addEventListener('submit', function(e) {
                ...
                Swal.fire({
                    ...
                    text: 'Apakah Anda yakin ingin memperbarui data jurnal mengajar ini?',
                    icon: 'question',
