...
@section('content')
    ...
    <div class="card">
        <div class="card-top-header">
            <div>
                <h2><i class="fa-solid fa-trash-can" style="color:#d97706;"></i> Tempat Sampah Wali Kelas ({{ count($trashedWali) }})</h2>
                <p>Daftar akun Wali Kelas yang dihapus sementara. Anda dapat memulihkan atau menghapus permanen.</p>
            </div>
            <a href="{{ route('admin.wali-kelas-list') }}" class="btn-back-main">
                <i class="fa-solid fa-arrow-left"></i> Kembali ke Daftar Wali Kelas
            </a>
        </div>
        ...
        <div class="table-responsive">
            <table class="table-custom">
                ...
                <tbody>
                    @forelse($trashedWali as $index => $u)
                        <tr>
                            ...
                            <td style="text-align:center;">
                                <div style="display:inline-flex; gap:6px;">
                                    <form action="{{ route('admin.wali-kelas.restore', $u->id) }}" method="POST" style="display:inline-block;">
                                        @csrf
                                        <button type="submit" class="btn-act btn-restore" title="Pulihkan Wali Kelas">
                                            <i class="fa-solid fa-rotate-left"></i> Pulihkan
                                        </button>
                                    </form>

                                    <form action="{{ route('admin.wali-kelas.force-delete', $u->id) }}" method="POST" style="display:inline-block;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn-act btn-force-delete" onclick="return confirm('Hapus PERMANEN Wali Kelas {{ addslashes($u->name) }}? Data tidak bisa dikembalikan lagi!')" title="Hapus Permanen">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" style="text-align:center; padding:36px; color:#94a3b8;">
                                ...
                                Tempat sampah kosong. Tidak ada data Wali Kelas yang dihapus.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
