...
@section('title', 'Data Jurusan — EduJournal Admin')
@section('header_title', 'Kelola Data Jurusan')
@section('header_subtitle', 'Manajemen daftar program keahlian / jurusan sekolah')
    ...
    <div class="action-header">
        <div style="display: flex; gap: 10px;">
            <a href="{{ route('jurusan.create') }}" class="btn-create">
                <i class="fa-solid fa-plus"></i> Tambah Data Jurusan
            </a>
            <a href="{{ route('jurusan.trash') }}" class="btn-trash-view">
                <i class="fa-solid fa-trash-can"></i> Lihat Sampah Jurusan
            </a>
        </div>
    </div>
            ...
            <table class="custom-table">
                ...
                <tbody>
                    @forelse($jurusans as $j)
                        <tr>
                            <td><code>{{ $j->kode_jurusan }}</code></td>
                            <td><strong>{{ $j->nama_jurusan }}</strong></td>
                            <td>
                                <div style="display: flex; gap: 6px;">
                                    <a href="{{ route('jurusan.edit', $j->id_jurusan) }}" class="btn-icon btn-edit">
                                        <i class="fa-solid fa-pen"></i> Edit
                                    </a>
                                    <form action="{{ route('jurusan.destroy', $j->id_jurusan) }}" method="POST" style="display: inline-block;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn-icon btn-delete" onclick="return confirm('Pindahkan jurusan {{ $j->nama_jurusan }} ke sampah?')">
